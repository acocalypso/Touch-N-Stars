...
async function toggleMode() {
  if (cameraStore.cameraSettings.BinAverageEnabled) {
    try {
      const data = await apiService.cameraAction(`set-setting?settingName=BinAverageEnabled&newValue=false`);
    } catch (error) {
      console.log('Error:', error);
    }
  } else {
    try {
      const data = await apiService.cameraAction(`set-setting?settingName=BinAverageEnabled&newValue=true`);
    } catch (error) {
      console.log('Error:', error);
    }
  }
