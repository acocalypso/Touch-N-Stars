...
async function toggleMode() {
  if (cameraStore.cameraSettings.HighFullwellMode) {
    try {
      const data = await apiService.cameraAction(`set-setting?settingName=HighFullwellMode&newValue=false`);
    } catch (error) {
      console.log('Error:', error);
    }
  } else {
    try {
      const data = await apiService.cameraAction(`set-setting?settingName=HighFullwellMode&newValue=true`);
    } catch (error) {
      console.log('Error:', error);
    }
  }
