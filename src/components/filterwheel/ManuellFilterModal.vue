<template>
    <div>
      ...
      >
        ...
        >
          <div>
            <h2 class="text-xl font-bold mb-4">{{ $t('components.filterwheel.manual_filter.title') }}</h2>
            <p>{{ $t('components.filterwheel.manual_filter.name') }} {{ filterStore.filterName }}</p>
          </div>
          <div class="flex justify-center mt-auto">
            <button @click="confirmFilterChange" class="bg-cyan-500 text-white px-4 py-2 rounded hover:bg-cyan-600 transition duration-200">
              {{ $t('common.confirm') }}
            </button>
          </div>
        </div>
