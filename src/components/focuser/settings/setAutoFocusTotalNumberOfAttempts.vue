<template>
  <div class="flex flex-col md:flex-row w-full md:items-center min-w-28 border border-gray-500 p-1 rounded-lg">
    <label class="text-sm sm:text-xs md:mr-3 mb-2 md:mb-1 text-gray-200">{{
      $t('components.focuser.settings.AutoFocusTotalNumberOfAttempts') }}</label>
    <input
...
async function updateSetting() {
  ...
  try {
    const response = await apiService.profileChangeValue('FocuserSettings-AutoFocusTotalNumberOfAttempts', value);
...
onMounted(() => {
  setValue.value = store.profileInfo.FocuserSettings.AutoFocusTotalNumberOfAttempts;
});
</script>
