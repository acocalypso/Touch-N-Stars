                        ...
                        <div class="bg-gray-900/50 rounded-lg p-4 border border-gray-700">
                            <div class="space-y-3">
                                <div v-if="solveResult.Response?.SolveTime" class="flex justify-between items-center">
                                    ...
                                    <span class="text-cyan-400 font-semibold text-sm">{{ formatDateTime(solveResult.Response.SolveTime) }}</span>
                                </div>
                                <div v-if="solveResult.Response?.Orientation !== undefined" class="flex justify-between items-center">
                                    <span class="text-gray-400">Orientation:</span>
                                    <span class="text-cyan-400 font-semibold">{{ formatValue(solveResult.Response.Orientation) }}°</span>
                                </div>
                                <div v-if="solveResult.Response?.PositionAngle !== undefined" class="flex justify-between items-center">
                                    <span class="text-gray-400">Position Angle:</span>
                                    <span class="text-cyan-400 font-semibold">{{ formatValue(solveResult.Response.PositionAngle) }}°</span>
                                </div>
                                <div v-if="solveResult.Response?.Pixscale !== undefined" class="flex justify-between items-center">
                                    <span class="text-gray-400">Pixel Scale:</span>
                                    <span class="text-cyan-400 font-semibold">{{ formatValue(solveResult.Response.Pixscale) }}"</span>
                                </div>
                                <div v-if="solveResult.Response?.Radius !== undefined" class="flex justify-between items-center">
                                    <span class="text-gray-400">Radius:</span>
                                    <span class="text-cyan-400 font-semibold">{{ formatValue(solveResult.Response.Radius) }}</span>
                                </div>
                                <div class="flex justify-between items-center pt-2 border-t border-gray-600">
                                    <span class="text-gray-400">Flipped:</span>
                                    <span class="text-cyan-400 font-semibold">{{ solveResult.Response?.Flipped ? 'Yes' : 'No' }}</span>
                                </div>
                            </div>
                        </div>
