<template>
  <div v-if="isRotatorSelected" class="flex flex-col border border-gray-500 p-1 md:p-2 rounded-lg">
    <!-- Connection Mode -->
    <div class="flex flex-row items-center justify-between w-full">
      <label for="connectionMode" class="text-xs md:text-sm text-gray-200 mr-2">
        {{ $t('components.rotator.config.connectionMode') }}
      </label>
      <select
        id="connectionMode"
    ...
    <div v-if="connectionMode === 'CONNECTION_SERIAL'">
      ...
      <div class="flex flex-row items-center justify-between w-full mt-2 md:mt-3">
        <label for="devicePort" class="text-xs md:text-sm text-gray-200 mr-2">
          {{ $t('components.rotator.config.devicePort') }}
        </label>
        <select
          id="devicePort"
          v-model="devicePort"
      ...
      <div class="flex flex-row items-center justify-between w-full mt-2 md:mt-3">
        <label for="baudRate" class="text-xs md:text-sm text-gray-200 mr-2">
          {{ $t('components.rotator.config.baudRate') }}
        </label>
        <select
          id="baudRate"
    ...
    <div v-if="connectionMode === 'CONNECTION_TCP'">
      ...
      <div class="flex flex-row items-center justify-between w-full mt-2 md:mt-3">
        <label for="ipAddress" class="text-xs md:text-sm text-gray-200 mr-2">
          {{ $t('components.rotator.config.ipAddress') }}
        </label>
        <input
          id="ipAddress"
          v-model="ipAddress"
      ...
      <div class="flex flex-row items-center justify-between w-full mt-2 md:mt-3">
        <label for="tcpPort" class="text-xs md:text-sm text-gray-200 mr-2">
          {{ $t('components.rotator.config.tcpPort') }}
        </label>
        <input
          id="tcpPort"
          v-model="tcpPort"
...
const props = defineProps({
  selectedRotatorDevice: { type: String, default: '' },
});

const store = apiStore();

const isRotatorSelected = computed(() => {
  const deviceName = props.selectedRotatorDevice.toLowerCase();
  const driverInfo = store.rotatorInfo?.DriverInfo?.toLowerCase() || '';
  const category = store.rotatorInfo?.Category?.toLowerCase() || '';
...
const initializeSettings = () => {
  if (!store.profileInfo?.RotatorSettings) {
    console.warn('RotatorSettings not loaded');
    return;
  }

  connectionMode.value =
    store.profileInfo.RotatorSettings.RotatorConnectionMode ?? 'CONNECTION_SERIAL';
  devicePort.value = store.profileInfo.RotatorSettings.RotatorPort ?? '/dev/ttyUSB0';
  baudRate.value = store.profileInfo.RotatorSettings.RotatorBaudRate ?? 9600;
  ipAddress.value = store.profileInfo.RotatorSettings.RotatorAddress ?? 'localhost';
...
async function setConnectionMode() {
  try {
    await apiService.profileChangeValue(
      'RotatorSettings-RotatorConnectionMode',
      connectionMode.value
    );
...
async function setDevicePort() {
  try {
    await apiService.profileChangeValue('RotatorSettings-RotatorPort', devicePort.value);
...
async function setBaudRate() {
  try {
    await apiService.profileChangeValue('RotatorSettings-RotatorBaudRate', baudRate.value);
...
async function setIPAddress() {
  try {
    await apiService.profileChangeValue('RotatorSettings-RotatorAddress', ipAddress.value);
...
async function setTCPPort() {
  try {
    await apiService.profileChangeValue('RotatorSettings-RotatorPort', tcpPort.value);
