  ...
  <ItemShell :item="item">
    ...
    <template #editor>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-1">
        <NumberInputPicker :modelValue="coords.RAHours"   label="h" labelKey="dso-ra-h" :min="0" :max="23" :step="1"                       labelPosition="top" wrapperClass="w-full" @change="saveRa($event,             coords.RAMinutes, coords.RASeconds)" />
        <NumberInputPicker :modelValue="coords.RAMinutes" label="m" labelKey="dso-ra-m" :min="0" :max="59" :step="1"                       labelPosition="top" wrapperClass="w-full" @change="saveRa(coords.RAHours, $event,               coords.RASeconds)" />
        <NumberInputPicker :modelValue="coords.RASeconds" label="s" labelKey="dso-ra-s" :min="0" :max="59" :step="0.1" :decimalPlaces="1" labelPosition="top" wrapperClass="w-full" @change="saveRa(coords.RAHours, coords.RAMinutes,  $event)" />
      </div>

      <!-- Dec -->
      <div class="text-xs text-slate-400 font-medium pt-1">{{ $t('components.sequence.items.center.dec') }}</div>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-1">
        <NumberInputPicker :modelValue="decDeg"            label="°" labelKey="dso-dec-d" :min="-90" :max="90" :step="1"   labelPosition="top" wrapperClass="w-full" @change="saveDec($event,  coords.DecMinutes, coords.DecSeconds)" />
        <NumberInputPicker :modelValue="coords.DecMinutes" label="m" labelKey="dso-dec-m" :min="0"   :max="59" :step="1"   labelPosition="top" wrapperClass="w-full" @change="saveDec(decDeg, $event,        coords.DecSeconds)" />
        <NumberInputPicker :modelValue="coords.DecSeconds" label="s" labelKey="dso-dec-s" :min="0"   :max="59" :step="0.1" :decimalPlaces="1" labelPosition="top" wrapperClass="w-full" @change="saveDec(decDeg, coords.DecMinutes, $event)" />
      </div>
    </template>
  </ItemShell>
</template>
