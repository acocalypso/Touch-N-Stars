<template>
  <ItemShell :item="item">
    <template #summary>
      ...
      <span v-if="item.Expr?.Definition" class="text-xs text-slate-500 font-mono truncate">= {{ item.Expr.Definition }}</span>
    </template>
    ...
    <template #editor="{ save }">
      ...
      <div class="flex items-center gap-3">
        ...
        <input
          ...
          @change="save('Identifier', $event.target.value)"
        />
      </div>
      ...
      <NumberInputPicker
        ...
        :label="$t('components.sequence.items.expressionVariable.value')"
        labelKey="exprvar-value"
