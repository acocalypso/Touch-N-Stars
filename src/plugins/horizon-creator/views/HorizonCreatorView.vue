<template>
  <div class="horizon-creator flex flex-col h-full text-white p-3 gap-3 overflow-auto">
    <!-- Header -->
    <div class="flex items-center justify-between">
      <h1 class="text-lg font-semibold text-cyan-400">Horizon Creator</h1>
      <div class="flex gap-2">
        <label
          class="cursor-pointer px-3 py-1 text-sm bg-gray-700 hover:bg-gray-600 rounded border border-gray-600"
        >
          Load .hrz
    ...
    <div class="flex flex-col lg:flex-row gap-3 flex-1">
            ...
            <tbody>
              ...
              >
                ...
                <td class="py-1 px-1 text-right">
                  <button
                    @click.stop="horizonStore.removePoint(p)"
                    class="text-gray-600 hover:text-red-400 text-xs leading-none px-1"
                    title="Remove point"
                  >
                    ✕
                  </button>
        ...
        <div class="flex flex-col gap-1.5 pt-1 border-t border-gray-700">
          ...
          <div class="flex gap-1.5 items-end">
            ...
            <button
              ...
              :disabled="manualAz === '' || manualAlt === ''"
              class="px-3 py-1.5 text-sm bg-cyan-700 hover:bg-cyan-600 rounded font-medium disabled:opacity-40"
            >
              Add
            </button>
          </div>
      ...
      <div class="flex flex-col items-center gap-3 flex-1 min-w-0">
        <!-- Instructions -->
        <div
          class="w-full rounded-lg border border-gray-700 bg-gray-800/60 px-3 py-2 text-xs text-gray-400 space-y-1"
        >
          <p class="text-gray-200 font-medium">Define your local horizon</p>
          <p>
            Slew your mount to each obstruction point (rooftop, tree, wall) and click
            <span class="text-cyan-300">Add Mount Pos</span>. Repeat around the full 360°. When
            done, click <span class="text-indigo-300">Save to NINA</span> — NINA will restrict
            scheduling and meridian-flip logic to stay above your horizon.
          </p>
          <p class="text-gray-500">
            You can also click directly on the profile chart or type Az/Alt values manually in the
            list on the left.
          </p>
        </div>
        ...
        <div class="relative">
          ...
          >
            ...
            />
            <line
              v-for="az in [0, 45, 90, 135, 180, 225, 270, 315]"
              :key="az"
              :x1="CX"
              :y1="CY"
              ...
              stroke-width="1"
            />
            <text :x="CX" :y="CY - MAX_R - 6" text-anchor="middle" fill="#e2e8f0" font-size="11">
              N
            </text>
            <text :x="CX + MAX_R + 6" :y="CY + 4" text-anchor="start" fill="#e2e8f0" font-size="11">
              E
            </text>
            <text :x="CX" :y="CY + MAX_R + 14" text-anchor="middle" fill="#e2e8f0" font-size="11">
              S
            </text>
            <text :x="CX - MAX_R - 6" :y="CY + 4" text-anchor="end" fill="#e2e8f0" font-size="11">
              W
            </text>
            ...
            <!-- Pending marker -->
            <circle
              :cx="azAltToSvgX(pendingAz, pendingAlt)"
              :cy="azAltToSvgY(pendingAz, pendingAlt)"
              r="5"
              fill="none"
              stroke="#facc15"
              stroke-width="1.5"
            ...
            <g v-if="mountConnected && mountAlt != null">
              <line
                :x1="azAltToSvgX(mountAz, mountAlt) - 7"
                :y1="azAltToSvgY(mountAz, mountAlt)"
                :x2="azAltToSvgX(mountAz, mountAlt) + 7"
                :y2="azAltToSvgY(mountAz, mountAlt)"
                stroke="#22d3ee"
                stroke-width="1.5"
              />
              <line
                :x1="azAltToSvgX(mountAz, mountAlt)"
                :y1="azAltToSvgY(mountAz, mountAlt) - 7"
                :x2="azAltToSvgX(mountAz, mountAlt)"
                :y2="azAltToSvgY(mountAz, mountAlt) + 7"
                stroke="#22d3ee"
                stroke-width="1.5"
              />
            </g>
        ...
        <div class="w-full px-1">
          ...
          <div class="flex items-center gap-2 mb-1">
            <div class="flex-1" />
            <label
              class="cursor-pointer px-2 py-0.5 text-xs bg-gray-700 hover:bg-gray-600 rounded border border-gray-600 whitespace-nowrap"
            >
          ...
          <div class="flex items-start gap-0">
            ...
            >
              ...
              <!-- Mount position -->
              <line
                v-if="mountConnected && mountAlt != null"
                :x1="profX(mountAz)"
                :y1="PROF_T"
                :x2="profX(mountAz)"
                :y2="PROF_B"
            ...
            >
              ...
              <span style="font-size: 9px; color: #6b7280; line-height: 1.2; text-align: center"
                >Alt</span
              >
            </div>
          </div>
          ...
          <div
            v-if="panoramaUrl"
            class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs mt-1"
          ...
          >
            ...
            <input
              ...
              max="1"
              step="0.05"
              v-model.number="panoramaOpacity"
              class="w-24"
            />
            <span class="text-gray-400">Top</span>
            <input
              type="number"
              v-model.number="panoramaTopAlt"
              min="-30"
              max="90"
              step="5"
            ...
            <input
              type="number"
              v-model.number="panoramaBottomAlt"
              min="-30"
              max="90"
              step="5"
              class="w-14 bg-gray-700 rounded px-1 py-0.5 text-white border border-gray-600 outline-none text-center"
          ...
          <div class="flex items-center gap-2 mt-2" style="padding-right: 40px">
            ...
            <input
              ...
              max="360"
              step="0.5"
              v-model.number="pendingAz"
              class="flex-1"
            />
            <span class="text-xs font-mono text-yellow-300 w-14 text-right"
              >{{ pendingAz.toFixed(1) }}°</span
            >
            <span class="text-xs text-gray-400 ml-2">Alt</span>
            <span class="text-xs font-mono text-yellow-300 w-10 text-right"
              >{{ pendingAlt.toFixed(1) }}°</span
            >
          </div>
        ...
        >
          <div />
          <button
            @mousedown="mode === 'jog' && startJog('north')"
            @mouseup="mode === 'jog' && stopJog()"
            @mouseleave="mode === 'jog' && stopJog()"
            @touchstart.prevent="mode === 'jog' && startJog('north')"
            @touchend.prevent="mode === 'steps' ? stepSlew('north') : stopJog()"
            @touchcancel.prevent="stopJog()"
            @click="mode === 'steps' && stepSlew('north')"
            :disabled="!mountConnected || (mode === 'steps' && slewBusy)"
            class="dpad-btn"
          ...
          <div />
          <button
            @mousedown="mode === 'jog' && startJog('west')"
            @mouseup="mode === 'jog' && stopJog()"
            @mouseleave="mode === 'jog' && stopJog()"
            @touchstart.prevent="mode === 'jog' && startJog('west')"
            @touchend.prevent="mode === 'steps' ? stepSlew('west') : stopJog()"
            @touchcancel.prevent="stopJog()"
          ...
          >
            {{ mode === 'steps' ? selectedStep + '°' : '■' }}
          </button>
          <button
            @mousedown="mode === 'jog' && startJog('east')"
            @mouseup="mode === 'jog' && stopJog()"
            @mouseleave="mode === 'jog' && stopJog()"
            @touchstart.prevent="mode === 'jog' && startJog('east')"
            @touchend.prevent="mode === 'steps' ? stepSlew('east') : stopJog()"
            @touchcancel.prevent="stopJog()"
          ...
          <div />
          <button
            @mousedown="mode === 'jog' && startJog('south')"
            @mouseup="mode === 'jog' && stopJog()"
            @mouseleave="mode === 'jog' && stopJog()"
            @touchstart.prevent="mode === 'jog' && startJog('south')"
            @touchend.prevent="mode === 'steps' ? stepSlew('south') : stopJog()"
            @touchcancel.prevent="stopJog()"
        ...
        <div class="flex gap-2">
          <button
            class="flex-1 py-2 text-sm font-medium bg-gray-700 hover:bg-gray-600 rounded disabled:opacity-40"
            :disabled="loadingFromNina"
            @click="loadFromNina"
          >
            {{ loadingFromNina ? 'Loading…' : 'Load from NINA' }}
          </button>
          <button
            class="flex-1 py-2 text-sm font-medium bg-indigo-700 hover:bg-indigo-600 rounded disabled:opacity-40"
          ...
          <button
            class="shrink-0 py-2 px-3 text-sm font-medium bg-red-900 hover:bg-red-800 rounded disabled:opacity-40"
            :disabled="savingToNina"
            title="Reset NINA horizon to flat (no restriction)"
            @click="clearNinaHorizon"
          >
            Clear
          </button>
        </div>
        <div
          ...
          :class="ninaSaveOk ? 'text-green-400' : 'text-red-400'"
        >
          {{ ninaSaveStatus }}
        </div>
      </div>
...
function profX(az) {
  return PROF_L + az * (chartW.value / 360);
}
...
const profileLine = computed(() => {
  ...
  const interp = interpolateHorizon(horizonStore.points, 1);
  return (
    'M ' + interp.map((p) => `${profX(p.az).toFixed(1)},${profY(p.alt).toFixed(1)}`).join(' L ')
  );
});
...
function onLoadPanorama(event) {
  ...
  reader.onload = (e) => {
    panoramaUrl.value = e.target.result;
  };
...
async function stepSlew(direction) {
  ...
  switch (direction) {
    case 'north':
      alt = Math.min(89, alt + step);
      break;
    case 'south':
      alt = Math.max(-30, alt - step);
      break;
    case 'east':
      az = (az + step + 360) % 360;
      break;
  ...
  try {
    ...
    console.error('Horizon step-slew error:', e);
  } finally {
...
async function clearNinaHorizon() {
  ...
  try {
    ...
    ninaSaveOk.value = true;
    ninaSaveStatus.value = 'Horizon cleared in NINA profile';
  } catch (e) {
    ninaSaveOk.value = false;
