<template>
  ...
  <ModalTransparanet :show="showConfiguration" @close="showConfiguration = false">
    <template #header>
      <div class="flex items-center justify-between w-full">
        <h2 class="text-1xl font-semibold">{{ t('plugins.livestack.configuration_title') }}</h2>
        <div class="flex items-center gap-2 text-gray-400">
          ...
          <span class="text-xs">{{ t('plugins.livestack.drag_hint') }}</span>
        </div>
      </div>
    </template>
    <template #body>
      <div class="flex flex-row items-center justify-between w-full p-2">
        <label for="toggle_osc_components" class="text-gray-200">
        ...
        <div>
          <toggleButton
            ...
            class="pr-5 pl-5 justify-center z-40"
          />
        </div>
      </div>
    </template>
