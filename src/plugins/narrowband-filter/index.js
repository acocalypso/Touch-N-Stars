...
import FilterCalculatorView from './views/FilterCalculator.vue';
import { usePluginStore } from '@/store/pluginStore';
import metadata from './plugin.json';
import FilterCalculatorIcon from './components/FilterCalculatorIcon.vue';
...
export default {
  ...
  install(app, options) {
    ...
    router.addRoute({
      path: pluginPath,
      component: FilterCalculatorView,
    ...
    if (currentPlugin && currentPlugin.enabled) {
      pluginStore.addNavigationItem({
        ...
        path: pluginPath,
        icon: markRaw(FilterCalculatorIcon),
        title: metadata.name,
      });
    }
  },
