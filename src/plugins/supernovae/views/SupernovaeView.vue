<template>
  <div class="px-4 py-6 space-y-4 text-gray-200">
    ...
    <div class="flex items-center justify-between gap-3 flex-wrap">
      ...
      >
        <svg v-if="store.downloading" class="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none">
          <circle
            class="opacity-25"
            cx="12"
            cy="12"
        ...
        {{
          store.downloading ? t('plugins.supernovae.downloading') : t('plugins.supernovae.download')
        }}
      </button>
    </div>
    ...
    <!-- Filters -->
    <div v-if="store.entries.length" class="flex flex-wrap gap-3 items-end">
      <!-- Mag filter -->
      <div class="flex flex-col gap-1">
        <label class="text-xs text-gray-400"
          >{{ t('plugins.supernovae.filterMag') }}
          <span class="text-white font-mono">≤ {{ magLimit }}</span></label
        >
        <input
          type="range"
      ...
      >
        <input type="checkbox" v-model="newOnly" class="accent-cyan-500 w-3.5 h-3.5" />
      ...
      >
        <input
          ...
          class="accent-green-500 w-3.5 h-3.5 disabled:opacity-40"
        />
        <span :class="hasLocation ? '' : 'opacity-40'">{{
...
const filtered = computed(() => {
  ...
  list = [...list].sort((a, b) => {
    ...
    if (sortKey.value === 'lastObserved') return b.lastObserved.localeCompare(a.lastObserved);
    return 0;
  });
...
function typeClass(type) {
  ...
  if (type.startsWith('Ia')) return 'bg-blue-900/50 text-blue-300';
  if (type.startsWith('II')) return 'bg-amber-900/50 text-amber-300';
  if (type.startsWith('Ib') || type.startsWith('Ic')) return 'bg-violet-900/50 text-violet-300';
