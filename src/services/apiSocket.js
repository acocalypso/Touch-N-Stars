...
class WebSocketLivestackService {
  constructor() {
    this.socket = null;
  ...
  connect() {
    ...
    console.log('Livestack WebSocket URL: ', this.backendUrl);

    this.socket = new WebSocket(this.backendUrl);

    this.socket.onopen = () => {
      console.log('WebSocket Livestack connected.');
    ...
    this.socket.onmessage = (event) => {
      try {
        ...
        console.error('Fehler beim Parsen der Livestack WebSocket Nachricht:', error);
        if (this.statusCallback) {
    ...
    this.socket.onerror = (error) => {
      console.error('Livestack WebSocket-Fehler:', error);
      if (this.statusCallback) {
    ...
    this.socket.onclose = () => {
      console.log('Livestack WebSocket geschlossen.');
      if (this.statusCallback) {
...
const websocketLivestackService = new WebSocketLivestackService();
export default websocketLivestackService;
